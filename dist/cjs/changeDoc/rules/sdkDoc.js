...
exports.changeDocSDKRules = void 0;
...
function breakingChange(type, isBreaking) {
...
function getNodeFromPath(source, path) {
    let res = source;
    path.forEach((item) => {
        res = res[item];
    });
...
const mark = (text) => "<code>" + text + "</code>";
...
const propertyRule = (target, location, index) => (data) => {
...
const parameterArrayChangeDoc = (data) => {
    let changedParamObj = [];
    let change = "";
    // Whole parameter object changed (parameter array)
    if (data.after)
        changedParamObj = changedParamObj.concat(data.after);
    if (data.before)
        changedParamObj = changedParamObj.concat(data.before);
    let isFirst = true;
...
const parameterChangeDoc = (data) => {
    ...
    if (data.action == "replace") {
        ...
        const param = (_a = data.before) !== null && _a !== void 0 ? _a : data.after;
        const isBreaking = param.required ? true : false;
...
const changeDocParametersRules = {
