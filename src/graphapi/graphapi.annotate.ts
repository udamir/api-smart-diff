...
const graphSchemaArgAnnotations = {
  ...
  add_target_schema:
    "[Added] {{text}} to Argument `{{target}}` of property `{{schema}}(...)`",
  add_target_directive:
    "[Added] {{text}} to Argument `{{target}}` of directive `@{{directive}}(...)`",
  remove: "[Removed] {{text}} from Argument {{target}}",
  remove_target: "[Removed] {{text}} from Argument `{{target}}`",
  remove_target_schema:
    "[Removed] {{text}} from Argument `{{target}}` of property `{{schema}}(...)`",
  remove_target_directive:
    "[Removed] {{text}} from Argument `{{target}}` of directive `@{{directive}}(...)`",
  replace: "[Replaced] {{text}} of Argument {{target}}",
  replace_target: "[Replaced] {{text}} of Argument `{{target}}`",
  replace_target_schema:
    "[Replaced] {{text}} of Argument `{{target}}` of property `{{schema}}(...)`",
  replace_target_directive:
    "[Replaced] {{text}} of Argument `{{target}}` of directive `@{{directive}}(...)`",
}
...
export const graphApiAnnotateHook: AnnotateHook = (diff, ctx) => {
  ...
  if (
    !annotate ||
    (diff.path[0] === "components" && diff.path[1] !== "directives")
  ) {
    return ""
  }
  if (isArgSchema(diff.path)) {
    ...
    const argsTemplate = {
      ...schemaChangeTemplate,
      params: { ...schemaChangeTemplate.params, schema, directive },
    }
    return createAnnotation(argsTemplate, {
      ...graphApiAnnotations,
      ...graphSchemaArgAnnotations,
    })
  }
...
export const valuesAnnotationChange: ChangeAnnotationResolver = (
  { path, action },
  ctx,
) => {
  ...
  switch (key) {
    ...
    case "reason":
      return t(action, {
        text: t("values_annotation", { key: "deprecation reason" }),
        target,
      })
  }
}

export const parentKeyChangeAnnotation: ChangeAnnotationResolver = (
  { path, action },
  ctx,
) => {
  const key = path[path.length - 1]
  ...
  switch (parentKey) {
    case "directives":
      return t(action, {
        text: t("directive", {
          key,
          definition: path[0] === "components" ? 1 : undefined,
        }),
        target,
      })
    case "deprecated":
