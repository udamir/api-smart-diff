...
describe("Openapi request body changes", () => {
  it("should compare requestBody", () => {
    ...
    expect(
      merged.paths["/pet"].put.requestBody.content["application/*"][metaKey],
    ).toMatchObject({
  ...
  it("should be non-breaking change to add not required property to requestBody", () => {
    ...
    expect(
      merged.paths["/pet"].put.requestBody.content["application/json"].schema
        .properties[metaKey],
    ).toMatchObject({
  ...
  it("should be non breaking change to remove required property to requestBody", () => {
    ...
    expect(
      ...
      required: {
        array: { 1: { action: DiffAction.remove, type: nonBreaking } },
      },
    })
