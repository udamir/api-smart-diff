...
describe("Test openapi 3 parameters compare", () => {
  ...
  it("should map query $ref parameters correctly", () => {
    ...
    expect(
      merged.paths["/test/endpoint"].get.parameters[0][metaKey],
    ).toMatchObject({
  ...
  it("should not add rename diff on query parameter name change", () => {
    ...
    expect(merged.paths["/pet/findByStatus"].get[metaKey]).toMatchObject({
      parameters: {
        array: {
  ...
  it("should classify operation parameter schema change", () => {
    ...
    expect(
      merged.paths["/pet/findByStatus"].get.parameters[0][metaKey],
    ).toMatchObject({
