...
export const paramClassifyRule: ClassifyRule = [
  ({ after }) =>
    getKeyValue(after.value, "required") && !isExist(getKeyValue(after.value, "schema", "default"))
      ? breaking
      : nonBreaking,
  breaking,
  unclassified,
...
export const operationSecurityClassifyRule: ClassifyRule = [
  ({ before, after }) =>
    emptySecurity(after.value) || includeSecurity(after.value, getKeyValue(before.root, "security"))
      ? nonBreaking
      : breaking,
