...
export const transformJsonSchemaCombiners =
  (allowedSibling = jsonSchemaAllowedSibling): CompareTransformResolver =>
  (before, after) => {
    ...
    for (const prop of ["oneOf", "anyOf"]) {
      if (
        prop in values.before &&
        !("oneOf" in values.after || "anyOf" in values.after)
      ) {
        return [
          values.before,
          createEmptyCombiner(values.after, prop, allowedSibling),
        ]
      }
      if (
        prop in values.after &&
        !("oneOf" in values.before || "anyOf" in values.before)
      ) {
        return [
          createEmptyCombiner(values.before, prop, allowedSibling),
          values.after,
        ]
      }
    }
...
export const createFields =
  (...fields: string[]): CompareTransformResolver =>
  (before, after) => {
    if (!isObject(before) || !isObject(after)) {
      return [before, after]
    }
...
export const transformJsonSchema =
  (version: "draft-04" | "2020-12" = "2020-12"): CompareTransformResolver =>
  (before, after) => {
    if (!isObject(before) || !isObject(after)) {
      return [before, after]
    }
    ...
    valuesTransformation(values, (value) => {
      ...
      if ("required" in value && Array.isArray(value.required)) {
        const required = value.required.filter(
          (v, i, arr) => isString(v) && arr.indexOf(v) === i,
        )
    ...
    valuesTransformation(values, (value, other) => {
      ...
      if ("items" in other && isArray(other.items)) {
        if ("items" in value && typeof value.items === "object") {
          return isArray(value.items)
            ? value
            : {
                ...value,
                items: other.items.map(() => value.items),
    ...
    valuesTransformation(values, (value, other) => {
      if (!("type" in value) && "type" in other) {
        ...
        if (types.length) {
          value.type =
            isString(other.type) && types.includes(other.type)
              ? other.type
              : types
        }
      }
...
export const jsonSchemaMergeAllOf = (version: "draft-04" | "2020-12") =>
  compareTransformationFactory((value) => {
    const draft = version === "draft-04" ? version : "draft-06"
    return merge(value, {
      rules: jsonSchemaMergeRules(draft),
