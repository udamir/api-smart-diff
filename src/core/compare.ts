...
export const createContext = (
  ...
  const { bNode, aNode, aPath, root, akey, bkey, bPath, before, after, rules } =
    data
...
export const createChildContext = (
  { before, after, rules, options }: ComapreContext,
  bkey: number | string,
  akey: number | string,
): ComapreContext => {
...
const useMergeFactory = (options: ComapreOptions = {}): MergeFactoryResult => {
  ...
  const hook: SyncCrawlHook<MergeState, CompareRule> = (crawlContext) => {
    ...
    const bkey =
      key ??
      (isArray(bNode) ? +Object.keys(keyMap).pop()! : Object.keys(keyMap).pop())
    ...
    const bPath =
      !state.bPath.length && bkey === "#" ? [] : [...state.bPath, bkey]
    const aPath =
      !state.aPath.length && akey === "#" ? [] : [...state.aPath, akey]

    // transform values before comparison
    const data: [unknown, unknown] = [value, aNode[akey]]
    const [before, after] =
      !isArray(value) && transform
        ? transform.reduce((res, t) => t(...res), data)
        : data
    ...
    const ctx = createContext(
      { ...state, before, after, akey, bkey, rules },
      options,
    )
    ...
    if (typeOf(before) !== typeOf(after)) {
      _diffs.push(
        setMergeMeta(
          parentMeta,
          akey,
          diffFactory.replaced(bPath, before, after, ctx),
        ),
      )
    ...
    if (isObject(before) && isObject(after)) {
      ...
      const mapKeys =
        mapping ??
        (isArray(before) ? arrayMappingResolver : objectMappingResolver)
      const { added, removed, mapped } = mapKeys(
        before as any,
        after as any,
        ctx,
      )
      const renamed = isArray(before)
        ? []
        : objectKeys(mapped).filter((key) => key !== mapped[key])
      ...
      _nodeDiffs.push(
        ...renamed.map((k) =>
          diffFactory.renamed(
            ...
            k,
            mapped[k],
            createChildContext(ctx, k, mapped[k]),
          ),
        ),
      )
      ...
      const exitHook = () => {
        ...
        if (!Object.keys(nodeMeta).length) {
          return
        }
...
export const compare: CompareEngine = (
  before: unknown,
  after: unknown,
  options: ComapreOptions = {},
  ...
  const { jsonPath: _bPath = [], source: bSource = before } =
    context.before ?? {}
  ...
  syncCrawl<MergeState, CompareRule>(before, hook, {
    state: rootState,
    rules: options.rules,
  })
  ...
  return {
    diffs,
    merged: root.merged[aKey],
    rootMergeMeta: rootState.parentMeta[aKey],
  }
}
