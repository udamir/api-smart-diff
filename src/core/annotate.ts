...
export const annotationTemplate = (
  template: string,
  params?: AnnotateTemplate["params"],
): AnnotateTemplate => ({
  template,
  ...(params ? { params } : {}),
})
...
export const createAnnotation = (
  annotationTemplate?: AnnotateTemplate,
  ...
  const findTemplate = (key: string, params: AnnotateTemplate["params"]) => {
    ...
    for (const _key of keys) {
      ...
      // find params
      if (
        !_params.filter((p) => !isExist(params?.[p]) || params?.[p] === "")
          .length &&
        _params.length >= matchCount
      ) {
        result = dict[_key]
  ...
  const applyTemplateParams = (
    name = "",
    _params: AnnotateTemplate["params"] = {},
  ) => {
    ...
    for (const key of objectKeys(_params)) {
      ...
      params[key] = isObject(param)
        ? createAnnotation(param as AnnotateTemplate, dict)
        : (param as string)
    }
    ...
    for (const match of [...template.matchAll(/{{(\w+)}}/g)].reverse()) {
      ...
      template =
        ...
        String(params[match[1]]) +
        template.substring(index + match[0].length)
    }
    return template
  }
